...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, dim, attn_type, head_num, ff_hidnum):
        ...
        builder = AttentionBuilder.from_kwargs(query_dimensions=dim)
        linear_attn = builder.get("linear")
        self.mhsa = AttentionLayer(attention=linear_attn, d_model=dim, n_heads=head_num)
    ...
    def forward(self, x):
        ...
        # k_len = FullMask(B,N,device=device)

        out = self.mhsa(queries=x, keys=x, values=x, attn_mask=mask, query_lengths=q_len, key_lengths=q_len)
        out = out + res
        out = self.ln1(out)
        res = torch.clone(out)
        out = self.ff(out)
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, dim, attn_type, head_num, ff_hidnum):
        ...
        builder1 = AttentionBuilder.from_kwargs(query_dimensions=dim)
        attn1 = builder1.get("causal-linear")
        self.causal_self_attn = AttentionLayer(attention=attn1, d_model=dim, n_heads=head_num)
        self.ln1 = nn.LayerNorm(dim)
        # self.mhsa = MultiHeadSourceAttention(dim, attn_type, head_num)
        builder2 = AttentionBuilder.from_kwargs(query_dimensions=dim)
        attn2 = builder2.get("linear")
        self.source_attn = AttentionLayer(attention=attn2, d_model=dim, n_heads=head_num)
        self.ln2 = nn.LayerNorm(dim)
        self.ff = FeedForward(dim, ff_hidnum)
    ...
    def forward(self, x, memory):
        ...
        res = torch.clone(x)
        # out = self.mhca(x)
        out = self.causal_self_attn(queries=x,keys=x,values=x,attn_mask=mask_tt,query_lengths=t_len,key_lengths=t_len)

        out = out + res
        out = self.ln1(out)
        res = torch.clone(out)

        out = self.source_attn(queries=out,keys=memory,values=memory,attn_mask=mask_st,query_lengths=t_len,key_lengths=s_len)
